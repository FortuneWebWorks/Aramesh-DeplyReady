...
const DobTable = () => {
  ...
  const inputsValidator = (e) => {
    if (e.target.value.length >= 0 && e.target.value.length <= 2) {
      setInputs((prevInput) => {
        prevInput[
          e.target.parentNode.parentNode.nextSibling.getAttribute('value')
        ] = e.target.value;
        return prevInput;
      });
    } else {
  ...
  const setFamily = () => {
    ...
    if (children.length === 0) {
      setChildren((prevChildren) => {
        ...
        for (let daughter = 0; daughter < Number(daughterNum); daughter++) {
  ...
  const blurHandler = (e) => {
    if (e.target.value === '') {
      if (e.target.parentElement.nextElementSibling) {
        e.target.parentElement.nextElementSibling.classList.remove('focus');
      }
    }
  };
        ...
        setParentsNodes((prevParents) => {
          prevParents.push(
            <tr key={Math.random()}>
              ...
              <td>
                <DropDown data={parents} title="والد را انتخاب کنید" />
              </td>
            </tr>
          );
          return prevParents;
  ...
  const emptyInputs = () => {
    ...
    for (let i = 0; i < ageOfBirths.length; i++) {
      if (ageOfBirths[i].value === '') {
        return false;
      } else if (i === ageOfBirths.length - 1 && ageOfBirths[i].value !== '') {
        return true;
      }
    }
    return true;
      ...
      setFormData((prevData) => {
        for (let i = 1; i < formRows.length; i++) {
          prevData.push({
            ...
            birthDate:
              Number(
                formRows[i].childNodes[0].childNodes[0].childNodes[1].value
              ) + 1300,
          });
        }
        return prevData;
  ...
  const labelChildren = () => {
    if (Number(familyNum) - Number(sonsNum) - Number(daughterNum) > 1) {
      setFormData((prevData) => {
        for (let i = 2; i < prevData.length; i++) {
          prevData[i].role = prevData[i].role.concat(' - ', ordinalNums[i - 2]);
        }
        return prevData;
      });
    } else {
      setFormData((prevData) => {
        for (let i = 1; i < prevData.length; i++) {
          prevData[i].role = prevData[i].role.concat(' - ', ordinalNums[i - 1]);
        }
        return prevData;
      });
    }
    ...
    setTimeout(() => {
      if (Number(familyNum) - (Number(sonsNum) + Number(daughterNum)) !== 1) {
        ...
        const formRows = document.querySelectorAll('tr');
        if (
          !formRows[1].childNodes[1].childNodes[0].childNodes[0]
            .getAttribute('value')
            .includes('پدر') &&
          !formRows[1].childNodes[1].childNodes[0].childNodes[0]
            .getAttribute('value')
  ...
  return (
    <div className="bg-height-100 bg-dark signup-table">
      <Card>
        ...
        <p className="guide">لطفا سال تولد هر عضو خانواده را وارد کنید</p>
          ...
          <div className="table-container">
            <table className="table" bordercollapse="collapse">
              ...
              <tbody>
                ...
                {childrenNodes.map((childrenNode) => childrenNode)}
              </tbody>
            </table>
          </div>

        ...
        <tbody>
            ...
            <>
              ...
              {props.integration.map((integration, index) => {
                ...
                return (
                  index >= parentNumber &&
                  index <
                    props.children.length * props.parents.length +
                      parentNumber && (
                    <tr key={`parent-children-role-${index}`}>
          ...
          {props.children.length > 1 && (
            <>
              <tr>
                <td
                  rowSpan={props.children.length * (props.children.length - 1)}
                >
                  CHILD
                  <br />/<br />
                  CHILD
                  ...
                  index <
                    props.children.length * props.parents.length +
                      ...
                      (props.children.length * (props.children.length - 1)) /
                        2 && (
                    <tr key={`children-children-role-${index}`}>
                      <td>{integration.role}</td>
                      <td>{integration.integration}</td>
                    </tr>
                  )

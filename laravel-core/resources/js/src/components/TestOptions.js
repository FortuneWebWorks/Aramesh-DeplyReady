...
const TestOptions = ({name, answer, setAnswer, currentPage, dataLimit}) => {
  ...
  const onchangeHandler = (e) => {
    const container = e.target.parentNode.parentNode.parentNode.parentNode.parentNode;
    
    setAnswer((userChoice) => {
      userChoice[(currentPage-1)*dataLimit + Number(container.id) + 1] = Number(e.target.value);
      return userChoice;
    }); 
  }
  ...
  return (
    <div className="options-container">
      <div className="radio-btns-container">
        ...
        <div className="radio-btn-container">
          <input type="radio" value={4} name={name} checked={answer === 4} onChange={onchangeHandler}/>
          <span className="radio-control">
            <img src={check} alt="Check icon" className="check-icon"/>
          </span>
        </div>
